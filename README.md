...
| `successor` | Mint the next session's id, point it at your handoff, and hand over. `--handoff <path>`, `--task ".."`, `--title ".."`, `--cwd <dir>`, `--session-id <id>`, `--dry-run`. Set `limits.successor.skip_permissions: true` to launch it with `--dangerously-skip-permissions` — a config key, never a flag. |
