...
| `mandate --set` / `--clear` / `--park` | Bind, release, or pause a job it cannot abandon. `--park` is you calling a break.  `--wake-path "<how a signal reaches this session>"` records that something OUTSIDE can reach the run, and `--wake-every <minutes>` records how often it should fire — with a cadence declared, `status` reports a path that has STOPPED delivering, which one missed wake could never show; `status` says so when nothing is recorded, because every other gate fires from inside the session. A woken run records its arrival with `note --woke`, and `status` then reports when the last wake **landed** and how long ago — the only direction visible from inside. A wake that was requested and never delivered leaves nothing here, which is why the declaration is still not a probe. |
| `mandate --set ".." --because "<the instruction, verbatim>"` | Records the instruction that **prompted** the mandate, beside it. Optional and **unverified** — nothing can check you did not paraphrase, so it is logged as `because_quote` with `because_verified: false` and no consumer may read it as established. It exists because one consumer's log held 29 mandates and 0 with the triggering instruction recorded, which made a whole class of question answerable on another repo's data and not on theirs. **And say what finishing looks like:** measured across 35 mandates in five repos, when a finish condition is there at all it is in the *agent's* words — so a mandate with no boundary is a decision you made and did not write down, not a gap in the brief you were given. |
| `doorbell` | Print the wake-up prompt for **this** run — what done means, where to resume, and the recovery paths recorded with `note --recovery ".."` — to paste into whatever pokes the session from outside. **Generated, not a template.** The paths that make a wake produce progress instead of re-orientation are per-run, so a static file you fill in is the thing that does not work. It says so plainly when it has none to carry, and when nothing records how the prompt is delivered — a doorbell nobody rings reads, from in here, exactly like one that never had to ring. |
...
| `authorize --revoke <path> [--reason ".."]` | Withdraw a grant you no longer need. It is **disarmed, not deleted**: the balance goes to zero and the record keeps what the grant was for, because "logged forever" is the whole value of the hatch and deleting takes the evidence with it. A revoke that matches nothing **refuses** rather than reporting success — a success line over an empty match set leaves the hatch armed while the record says it was closed. `status` counts revoked grants apart from spent ones. |
| `authorize --path <prefix> --reason ".." --expires <when>` | Bound the grant in **time** as well as in path and count. `30m` / `2h` / `7d` / `1w`, a date (`2026-09-30`, meaning the end of that day), or `never` — which stays the default. After it lapses the guard refuses the call and **spends nothing**. A bare number is refused: read as minutes it lapses during the call that set it, read as days it covers a week of unattended runs, and nothing downstream can tell which you meant. Time is the one dimension of this hatch that acts while nobody is looking, which is exactly when a stale grant is dangerous; `status` prints a live grant's age once it is more than a day old, and says when it has no expiry. |
