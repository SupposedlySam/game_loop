...
MUTANTS += [
    ...
    ('assertions_that_cannot_fail -> no assertion looks unfailable',
     ...
     "2 kills, MEASURED by the full sweep of 2026-08-26 against d2411ab — the first reading this producer has ever had. It entered the denominator with #115's widening of the candidate finder, and its first two sweeps produced NOTHING: the declared mutant body carried a literal backslash-n instead of a newline, so the file did not parse and the suite could not start. Comparable: test/run.py is unchanged between that sweep's tree and this commit. FIVE ASSERTIONS ADDED 2026-08-27, after the killers file showed only ONE of its three kills was its own: the other two are this sweep's universal bookkeeping, one of which reddens for all 132 producers. A single assertion covered the check that catches assertions which cannot fail. The new ones are all something-direction, because the existing else-False arm expects [] and the mutant returns [] — it can never contribute. Four of the five were confirmed to kill a neutered build; the fifth (a ternary outside a check is ignored) survives by construction and was expected to. FLOOR LEFT AT 2 until a sweep sees them; expect roughly 5 and raise it from the killers file.", 7),
]
